    ...
    <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-50 px-4">
      <div className="max-w-7xl mx-auto h-20 md:h-24 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={onCartClick}
            className="relative p-3 bg-orange-50 rounded-2xl transition-all cursor-pointer active:scale-90 hover:bg-orange-100"
          >
            <ShoppingBag className="w-6 h-6 text-orange-600" />
            {totalItems > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center border-2 border-white font-bold animate-bounce-short">
                {totalItems}
              </span>
            )}
          </button>
